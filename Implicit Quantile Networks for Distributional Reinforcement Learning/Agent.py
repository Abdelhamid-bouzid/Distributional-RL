...
class Agent(object):
    ...
    def choose_action(self, state):
        ...
        if np.random.random()> self.epsilon:
            ...
            Q,_       = self.eval_model.forward(state)   #(batch_size,n_actions,n_quants)
    ...
    def learn(self):
        ...
        Q_pred, Q_tau = self.eval_model(states)    #(batch_size,n_actions,n_quants)
        Q_pred        = Q_pred[indices,actions,:]  #(batch_size,n_quants)
        Q_pred        = Q_pred.unsqueeze(2)        #(batch_size,n_quants, 1)
        
        '''######################################### determine best action t###########################'''
        Q_next, Q_tau_next = self.next_model(n_states)        #(batch_size,n_actions, n_quants)
        ...
        QUANTS_TARGET = Q_tau.unsqueeze(0) 		                             # (1, n_quants, 1)
